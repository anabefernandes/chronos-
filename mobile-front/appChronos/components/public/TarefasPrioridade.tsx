  ...
  const renderPrioridade = (prio: 'alta' | 'media' | 'baixa') => {
    ...
    return (
      <View
        style={{
          flex: 1,
          justifyContent: tarefasExibidas.length === 0 ? 'center' : 'flex-start',
          marginBottom: 24
        }}
      >
        ...
        {tarefasExibidas.length > 0 ? (
          ...
          <View style={styles.tarefasVaziasContainer}>
            <LottieView
              source={require('../../assets/lottie/sem_tarefas2.json')} // JSON para prioridade vazia
              autoPlay
              loop
